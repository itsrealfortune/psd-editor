...
import { findLayerByName, getLinkedFile, printLayerTree, inspectLayers } from "./layers.js";
import type { ExportOptions, ImageReplacement, TemplateReplacements, TextReplacement } from "./types.js";

// ─── Text replacements ────────────────────────────────────────────────────────

function applyTextReplacements(psd: Psd, replacements: TextReplacement[]): void {
  for (const rep of replacements) {
    const layer = findLayerByName(psd, rep.layerName);

    if (!layer) {
      console.warn(`[psd-template] ⚠ Text layer "${rep.layerName}" not found — skipping.`);
      continue;
    }
    if (!layer.text) {
      console.warn(`[psd-template] ⚠ Layer "${rep.layerName}" is not a text layer — skipping.`);
      continue;
    }
    ...
    if (rep.fontSize !== undefined || rep.color !== undefined) {
      ...
      if (rep.color !== undefined) {
        layer.text.style.fillColor = { r: rep.color[0], g: rep.color[1], b: rep.color[2] };
      }
      ...
      if (layer.text.styleRuns) {
        for (const run of layer.text.styleRuns) {
          ...
          if (rep.color !== undefined) {
            run.style.fillColor = { r: rep.color[0], g: rep.color[1], b: rep.color[2] };
          }
        }
      }
    }

    console.log(`[psd-template] ✓ Text "${rep.layerName}" → "${rep.text.slice(0, 40)}"`);
  }
}

// ─── Image replacements ───────────────────────────────────────────────────────

async function applyImageReplacements(psd: Psd, replacements: ImageReplacement[]): Promise<void> {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { createCanvas, createImageData } = require("canvas") as typeof import("canvas");
  ...
  for (const rep of replacements) {
    ...
    if (!layer) {
      console.warn(`[psd-template] ⚠ Image layer "${rep.layerName}" not found — skipping.`);
      continue;
    }
    ...
    if (w <= 0 || h <= 0) {
      console.warn(`[psd-template] ⚠ Layer "${rep.layerName}" has zero bounds — skipping.`);
      continue;
    }
    ...
    ctx.putImageData(createImageData(new Uint8ClampedArray(data), info.width, info.height), 0, 0);
    ...
    if (linkedFile) {
      ...
      console.log(`[psd-template] ✓ Smart object "${rep.layerName}" linked file replaced.`);
    }

    console.log(`[psd-template] ✓ Image "${rep.layerName}" canvas replaced (${w}×${h}).`);
  }
}
...
export class PsdTemplate {
   ...
   */
  async render(replacements: TemplateReplacements, options: ExportOptions): Promise<void> {
    ...
    try {
      ...
      await pipeline.png({ compressionLevel: options.compressionLevel ?? 6 }).toFile(options.outputPath);
